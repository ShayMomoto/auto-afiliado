...
// --- SERVICE LAYER ---
class ApiService {
  constructor() { this.baseUrl = "http://localhost:5000/api"; }
  ...
  // AGORA CHAMA A ROTA REAL DE SCRAPING
...
// ... (Constantes NICHES, TONES mantidas) ...
const NICHES = { mix: { label: "🔥 Mix", keywords: "promoção" }, tech: { label: "💻 Tech", keywords: "notebook" } };
...
// --- TELA DE AGENDAMENTO MANUAL (COM FOTO) ---
const ManualSchedulingScreen = ({ showToast }) => {
  ...
  const handleLinkChange = (e) => {
    ...
    if (url.includes('http')) { setLinkStatus('valid'); setDetectedStore('Link Detectado'); }
    else { setLinkStatus('warning'); }
  };
  ...
  const handleAnalyzeLink = async () => {
    ...
    setIsAnalyzing(true);
    try {
      // Agora chama o backend REAL que faz scraping
      const res = await api.current.analyzeLink(manualLink);
      
      if(res.success) {
        const item = res.data;
        // Gera copy usando os dados reais extraídos (incluindo imagem)
        const fakeCopy = `🔥 OFERTA ENCONTRADA!\n\n${item.item_name}\n\n💰 ${item.price !== 'Consulte' ? 'Por apenas: '+item.price : 'Preço Especial'}\n\n👇 Compre aqui:\n${item.affiliate_link}`;
        
        setPreviewItem({ ...item, finalMessage: fakeCopy });
        showToast("Dados e Imagem extraídos!", "success");
      } else {
        showToast("Não foi possível ler o site.", "error");
      }
    } catch (error) { 
  ...
  const handleAddSlot = () => {
    if (!scheduleDate || !scheduleTime) { showToast("Defina data e hora.", "error"); return; }
        ...
        <NeonCard className="border-indigo-500/20">
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="relative">
                <div className={`flex items-center gap-2 bg-slate-950 border rounded-lg px-3 py-2 transition-colors ${linkStatus === 'valid' ? 'border-emerald-500/50' : 'border-slate-700'}`}>
                    <input value={manualLink} onChange={handleLinkChange} placeholder="Cole o link aqui..." className="w-full bg-transparent border-none text-sm text-white focus:outline-none" />
                    {linkStatus === 'valid' && <CheckCircle size={18} className="text-emerald-500 animate-in zoom-in" />}
                </div>
              </div>
            </div>

            <button onClick={handleAnalyzeLink} disabled={isAnalyzing || !manualLink} className="w-full mt-2 bg-slate-800 border border-slate-700 text-white p-2 rounded-lg hover:bg-slate-700 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors">{isAnalyzing ? <Loader size={16} className="animate-spin"/> : <Search size={16}/>}<span>Buscar Dados</span></button>

            {/* PREVIEW DO PRODUTO (COM FOTO) */}
            {previewItem && (
              <div className="p-3 bg-slate-900/50 rounded-lg border border-white/5 animate-fade-in mt-4">
                <div className="flex gap-3 mb-3">
                   {/* ÁREA DA FOTO */}
                   <div className="w-16 h-16 bg-white rounded-lg flex items-center justify-center overflow-hidden shrink-0 border border-slate-700">
                      {previewItem.image_url && !previewItem.image_url.includes('placeholder') ? (
                        <img src={previewItem.image_url} alt="Produto" className="w-full h-full object-contain" />
                      ) : (
                ...
                <p className="text-xs text-emerald-400 font-bold mb-2 pl-1">Preço: {previewItem.price}</p>
            ...
            <div className="space-y-4 pt-4 border-t border-white/5">
              <p className="text-xs font-bold text-slate-400 uppercase flex items-center gap-2"><CalendarClock size={14}/> Agendar Horários</p>
              
              <div className="flex gap-2">
                <input type="date" value={scheduleDate} onChange={(e) => setScheduleDate(e.target.value)} onClick={(e) => e.target.showPicker && e.target.showPicker()} className="bg-slate-950 border border-slate-700 rounded-lg px-2 py-2 text-xs text-white flex-1 focus:border-indigo-500 outline-none cursor-pointer" />
                <input type="time" value={scheduleTime} onChange={(e) => setScheduleTime(e.target.value)} onClick={(e) => e.target.showPicker && e.target.showPicker()} className="bg-slate-950 border border-slate-700 rounded-lg px-2 py-2 text-xs text-white w-24 focus:border-indigo-500 outline-none cursor-pointer"/>
              ...
              {scheduledSlots.length > 0 && (
                <div className="bg-slate-900/50 rounded-lg p-2 max-h-[100px] overflow-y-auto space-y-1 scrollbar-hide border border-slate-800">
                  {scheduledSlots.map((slot) => (<div key={slot.id} className="flex justify-between items-center bg-slate-800 px-3 py-1.5 rounded text-xs animate-fade-in"><span className="text-indigo-300 font-mono">{slot.date.split('-').reverse().join('/')} às {slot.time}</span><button onClick={() => setScheduledSlots(s => s.filter(x => x.id !== slot.id))} className="text-slate-500 hover:text-red-400"><X size={12}/></button></div>))}
                </div>
              )}
            </div>
                 ...
                 <div key={item.id} className="p-4 bg-slate-900/40 rounded-xl border border-white/5 hover:border-indigo-500/30 transition-all group relative animate-fade-in">
                   <div className="flex justify-between items-start">
                     <div className="flex items-start gap-4">
                       {/* FOTO NO CARD DA LISTA */}
                       <div className="w-12 h-12 bg-white rounded-lg flex items-center justify-center overflow-hidden shrink-0 border border-slate-700">
                       ...
                       <div>
                         ...
                         <p className="text-xs text-emerald-400 font-bold mt-0.5">{item.price}</p>
...
// --- (RESTO DO APP - AutoPilotScreen, Dashboard, App Main - IGUAL AO ANTERIOR) ---
