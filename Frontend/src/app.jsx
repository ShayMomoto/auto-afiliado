...
class ApiService {
  constructor() { 
    ...
    // O ambiente de preview não suporta 'import.meta' diretamente.
    // Para produção (Vercel), descomente a linha abaixo e comente a seguinte:
    // this.baseUrl = import.meta.env.VITE_API_URL || "http://localhost:5000/api";
    
    this.baseUrl = "http://localhost:5000/api"; 
    
    console.log("🔗 Conectando ao Backend em:", this.baseUrl);
  }
  ...
  // FUNÇÃO ATUALIZADA: Simulação de Scraping
  async analyzeLink(link) {
    try {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          let storeName = "Oferta";
          let productName = "Produto Detectado";
  
          const l = link.toLowerCase();
          if (l.includes('shopee')) { storeName = "Shopee"; productName = "Cadeira Gamer Ergonômica"; }
          else if (l.includes('amazon')) { storeName = "Amazon"; productName = "Echo Dot 4ª Geração"; }
          else if (l.includes('magalu')) { storeName = "Magalu"; productName = "Smartphone Samsung S23"; }
          else if (l.includes('mercadolivre')) { storeName = "Mercado Livre"; productName = "Kit Ferramentas"; }
  
          resolve({
            success: true, 
            data: {
              item_name: `${storeName} - ${productName}`,
...
// --- CONSTANTES ---
const NICHES = { mix: { label: "🔥 Mix de Ofertas", keywords: "promoção" }, tech: { label: "💻 Tech", keywords: "notebook" }, home: { label: "🏠 Casa", keywords: "airfryer" }, beauty: { label: "💄 Beleza", keywords: "skincare" } };
const TONES = { urgent: { label: "🚨 Urgente", icon: AlertTriangle, color: "text-red-400" }, friendly: { label: "🥰 Amigável", icon: MessageCircle, color: "text-pink-400" }, luxury: { label: "💎 Luxo", icon: DollarSign, color: "text-purple-400" }, minimalist: { label: "📉 Minimalista", icon: Activity, color: "text-slate-400" } };

// --- STYLES ---
const GlobalStyles = () => (
  <style>{`
    .scrollbar-hide::-webkit-scrollbar { display: none; }
    .glass-panel { background: rgba(15, 23, 42, 0.7); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.08); box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1); }
    .animate-fade-in { animation: fadeIn 0.5s ease-out forwards; }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
    
    /* Configurações para inputs de data */
...
const ManualSchedulingScreen = ({ showToast }) => {
  ...
  const handleAnalyzeLink = async () => {
    ...
    try {
      ...
      if(res.success) {
        const fakeCopy = `🔥 OPORTUNIDADE!\n\n${res.data.item_name}\n\n💰 Por: R$ ${res.data.price}\n\n👇 Compre aqui:\n${res.data.affiliate_link}`;
        setPreviewItem({ ...res.data, finalMessage: fakeCopy });
        showToast("Link reconhecido com sucesso!", "success");
      } else { showToast("Não foi possível ler o link.", "error"); }
    } catch (error) { showToast("Erro de conexão.", "error"); } 
    finally { setIsAnalyzing(false); }
  };

  const handleAddSlot = () => {
    if (!scheduleDate || !scheduleTime) { showToast("Escolha uma data e horário.", "error"); return; }
    const newSlot = { id: Date.now(), date: scheduleDate, time: scheduleTime };
  ...
  const handleStartManual = () => {
    ...
    const newSchedules = scheduledSlots.map(slot => ({ 
      id: Math.random().toString(36).substr(2, 9), 
      ...previewItem, 
      scheduledAt: { date: slot.date, time: slot.time }, 
      status: 'pending', 
      ...
      <div className="space-y-6">
        <NeonCard className="border-indigo-500/20">
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
             ...
             <div><h3 className="text-lg font-bold text-white">Novo Agendamento</h3><p className="text-[10px] text-slate-400">Postagem Manual</p></div>
          </div>

          <div className="space-y-5">
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Link do Produto</label>
              <div className="relative">
                <div className={`flex items-center gap-2 bg-slate-950 border rounded-lg px-3 py-2 transition-colors ${linkStatus === 'valid' ? 'border-emerald-500/50' : 'border-slate-700'}`}>
                    <input value={manualLink} onChange={handleLinkChange} placeholder="Cole o link da loja..." className="w-full bg-transparent border-none text-sm text-white focus:outline-none" />
                    {linkStatus === 'valid' && <CheckCircle size={18} className="text-emerald-500 animate-in zoom-in" />}
                </div>
              </div>
            </div>

            <button onClick={handleAnalyzeLink} disabled={isAnalyzing || !manualLink} className="w-full bg-slate-800 border border-slate-700 text-white p-2 rounded-lg hover:bg-slate-700 disabled:opacity-50 flex items-center justify-center gap-2 transition-colors">
                {isAnalyzing ? <Loader size={16} className="animate-spin"/> : <Search size={16}/>}<span>Identificar Produto</span>
            </button>

            {previewItem && (
              <div className="p-3 bg-slate-900/50 rounded-lg border border-white/5 animate-fade-in mt-4">
                <div className="flex gap-3 mb-3">
                   <div className="w-12 h-12 bg-white rounded flex items-center justify-center border border-slate-700 overflow-hidden shrink-0">
                      {previewItem.image_url ? <img src={previewItem.image_url} className="w-full h-full object-contain"/> : <ImageIcon size={20} className="text-slate-400"/>}
                   </div>
                   <div>
                      <p className="text-xs text-slate-500 uppercase font-bold mb-1">Título Gerado</p>
                      <p className="text-sm font-bold text-white leading-tight line-clamp-2">{previewItem.item_name}</p>
                   </div>
                </div>
                <p className="text-xs text-emerald-400 font-bold mb-2">Preço: R$ {previewItem.price}</p>
            ...
            <div className="space-y-4 pt-4 border-t border-white/5">
              ...
              <div className="flex gap-2">
                <div className="flex-1 relative date-wrapper bg-slate-950 border border-slate-700 rounded-lg flex items-center">
                   <input 
                     type="date" 
                     value={scheduleDate} 
                     onChange={(e) => setScheduleDate(e.target.value)} 
                     className="w-full bg-transparent border-none px-2 py-2 text-xs text-white focus:outline-none cursor-pointer"
                   />
                </div>
                <div className="w-24 relative date-wrapper bg-slate-950 border border-slate-700 rounded-lg flex items-center">
                   <input 
                     type="time" 
                     value={scheduleTime} 
                ...
                <button onClick={handleAddSlot} className="bg-indigo-600 hover:bg-indigo-500 text-white p-2 rounded-lg transition-colors flex items-center justify-center"><Plus size={16} /></button>
              </div>

              {scheduledSlots.length > 0 && (
                <div className="bg-slate-900/50 rounded-lg p-2 max-h-[100px] overflow-y-auto space-y-1 scrollbar-hide border border-slate-800">
                  {scheduledSlots.map((slot) => (
                    <div key={slot.id} className="flex justify-between items-center bg-slate-800 px-3 py-1.5 rounded text-xs animate-fade-in border border-white/5">
                      <span className="text-indigo-300 font-mono flex items-center gap-2">
                        <Calendar size={10} /> {slot.date.split('-').reverse().join('/')} 
                        <span className="text-slate-600">|</span> 
                        <Clock size={10}/> {slot.time}
                      </span>
                      <button onClick={() => setScheduledSlots(s => s.filter(x => x.id !== slot.id))} className="text-slate-500 hover:text-red-400"><X size={12}/></button>
                    </div>
                  ))}
                </div>
              )}
                 ...
                 <div key={item.id} className="p-4 bg-slate-900/40 rounded-xl border border-white/5 hover:border-indigo-500/30 transition-all group relative animate-fade-in">
                   <div className="flex justify-between items-start">
                     <div className="flex items-start gap-4">
                       <div className="w-12 h-12 bg-slate-800 rounded-lg flex items-center justify-center text-2xl shrink-0 overflow-hidden border border-white/5">
                          {item.image_url ? <img src={item.image_url} className="w-full h-full object-cover"/> : "📦"}
                       </div>
                       <div>
                         <h4 className="text-sm font-bold text-white line-clamp-1">{item.item_name}</h4>
                         <p className="text-xs text-emerald-400 font-bold mt-0.5">R$ {item.price}</p>
                         <div className="flex items-center gap-3 mt-2 text-[11px] text-indigo-300 font-medium bg-indigo-500/10 w-fit px-2 py-1 rounded border border-indigo-500/20"><Calendar size={12}/> {item.scheduledAt.date.split('-').reverse().join('/')}<span className="text-slate-600">|</span><Clock size={12}/> {item.scheduledAt.time}</div>
                       </div>
                     </div>
...
// --- TELA DO PILOTO AUTOMÁTICO (SOMENTE AUTO) ---
const AutoPilotScreen = ({ updateStats, showToast }) => {
  const [isRunning, setIsRunning] = useState(false);
  const [botStatus, setBotStatus] = useState("idle");
  const [logs, setLogs] = useState([]);
  const [queue, setQueue] = useState([]);
  const [selectedNiche, setSelectedNiche] = useState('mix');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in pb-20">
      <div className="space-y-6">
        <NeonCard className="border-purple-500/20"><div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5"><div className="p-2 bg-purple-500/20 rounded-lg text-purple-400"><BrainCircuit size={20}/></div><div><h3 className="text-lg font-bold text-white">Piloto Automático (Pro)</h3><p className="text-[10px] text-slate-400">Mineração e postagem 100% autônoma</p></div></div><div className="space-y-5"><div><label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Nicho</label><div className="grid grid-cols-2 gap-2">{Object.entries(NICHES).map(([key, val]) => (<button key={key} onClick={() => setSelectedNiche(key)} className={`p-2 rounded-lg border text-xs font-medium transition-all ${selectedNiche === key ? 'bg-purple-600 border-purple-500 text-white' : 'bg-slate-900/50 border-slate-700 text-slate-400 hover:bg-slate-800'}`}>{val.label.split(' ')[0]} {val.label.split(' ')[1]}</button>))}</div></div><div><label className="text-xs font-bold text-slate-400 uppercase mb-2 block">Tom de Voz</label><div className="grid grid-cols-2 gap-2">{Object.entries(TONES).map(([key, val]) => (<button key={key} onClick={() => setSelectedTone(key)} className={`p-2 rounded-lg border text-xs font-medium transition-all ${selectedTone === key ? 'bg-purple-600 border-purple-500 text-white' : 'bg-slate-900/50 border-slate-700 text-slate-400'}`}>{val.label}</button>))}</div></div><button onClick={() => { setIsRunning(!isRunning); showToast(isRunning ? "Robô Pausado" : "Robô Iniciado", "success"); }} className={`w-full py-3 rounded-xl font-bold text-sm shadow-lg flex items-center justify-center gap-2 ${isRunning ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white'}`}>{isRunning ? <><Pause size={16}/> PAUSAR ROBÔ</> : <><Sparkles size={16}/> ATIVAR IA</>}</button></div></NeonCard>
      </div>
      <div className="lg:col-span-2 space-y-6">
